...
- [ ] 6. Phase 1 verification gate and commit
...
- [ ] 7. Phase 2: Cancellation migrations v1.10.0 through v1.10.9
  - **Blocked until task 6.2 records three zero results from one run.**

  - [~] 7.1 Write and apply `supabase/migrations/v1.10.0-cancellation-core-tables.sql`
  ...
  - [~] 7.2 Write and apply `supabase/migrations/v1.10.1-cancellation-templates.sql`
  ...
  - [~] 7.3 Write and apply `supabase/migrations/v1.10.2-cancellation-communications.sql`
  ...
  - [~] 7.4 Write and apply `supabase/migrations/v1.10.3-cancellation-case-activity.sql`
  ...
  - [~] 7.5 Write and apply `supabase/migrations/v1.10.4-cancellation-settings.sql`
  ...
  - [~] 7.6 Write and apply `supabase/migrations/v1.10.5-cancellation-import-batch.sql`
  ...
  - [~] 7.7 Write and apply `supabase/migrations/v1.10.6-cancellation-rls.sql`
  ...
  - [~] 7.8 Write and apply `supabase/migrations/v1.10.7-extend-user-notifications-type.sql`
  ...
  - [~] 7.9 Write and apply `supabase/migrations/v1.10.8-cancellation-evidence-bucket.sql`
  ...
  - [~] 7.10 Write and apply `supabase/migrations/v1.10.9-cancellation-seed-templates.sql`
...
- [ ] 8. RFC 4180 raw-row reader
  - [~] 8.1 Create `src/features/cancellations/import/csv.ts`
  ...
  - [~] 8.2 Write the CSV round-trip property test
...
- [ ] 9. Classification, mapping, preview, and the batch loader
  - [~] 9.1 Create `src/features/cancellations/import/classify.ts`
  ...
  - [~] 9.2 Create `src/features/cancellations/import/mapping.ts`
  ...
  - [~] 9.3 Create `src/features/cancellations/import/fields.ts` row-level parsers
  ...
  - [~] 9.4 Create `src/features/cancellations/import/preview.ts`
  ...
  - [~] 9.5 Create `src/features/cancellations/import/loader.ts`
  ...
  - [~] 9.6 Write import classification, parsing, and preview tests
...
- [ ] 10. Contact normalization and suppression
  - [~] 10.1 Create `src/features/cancellations/import/contacts.ts`
  ...
  - [~] 10.2 Write contact normalization tests
  ...
  - [~] 10.3 Create `src/features/cancellations/domain/suppression.ts`
...
- [ ] 11. Email delivery module
  - [~] 11.1 Create `src/lib/email.ts`
...
- [ ] 12. Message renderer and content gate
  - [~] 12.1 Create `src/features/cancellations/render/renderMessage.ts`
  ...
  - [~] 12.2 Create `src/features/cancellations/render/gate.ts` and wire it inside the renderer
  ...
  - [~] 12.3 Write the forbidden-token property test
  ...
  - [~] 12.4 Write renderer unit tests
...
- [ ] 13. Communication status derivation and escalation
  - [~] 13.1 Create `src/features/cancellations/domain/communication-status.ts`
  ...
  - [~] 13.2 Create `src/features/cancellations/domain/escalation.ts`
  ...
  - [~] 13.3 Write status and escalation tests
...
- [ ] 14. Notification scheduler
  - [~] 14.1 Create `src/app/api/cancellations/scheduler/route.ts`
  ...
  - [~] 14.2 Create `src/features/cancellations/scheduler/send.ts` reserve-then-send helper
  ...
  - [~] 14.3 Write the scheduler idempotency property test
  ...
  - [~] 14.4 Write scheduler unit tests
  ...
  - [~] 14.5 Write the provider isolation test
...
- [ ] 15. Manual send route
  - [~] 15.1 Create `src/app/api/cancellations/send/route.ts`
...
- [ ] 16. Cancellations UI
  - [~] 16.1 Create `src/features/cancellations/api.ts`
  ...
  - [~] 16.2 Create `src/features/cancellations/derive.ts`
  ...
  - [~] 16.3 Write cancellations derive tests
  ...
  - [~] 16.4 Create `src/features/cancellations/CancellationsSummaryBar.tsx`
  ...
  - [~] 16.5 Create `src/features/cancellations/CancellationsTable.tsx`
  ...
  - [~] 16.8 Create `src/features/cancellations/CancellationContactPanel.tsx`
...
**Phase 2 gate (task 19.1):** run id _(pending)_ — `npx tsc --noEmit`: _(pending)_ · `npm test`: _(pending)_ · `npm run test:integration`: _(pending)_ · `npm run build`: _(pending)_
