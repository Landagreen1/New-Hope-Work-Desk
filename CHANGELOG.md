...
- The Work Desk asks Microsoft for four delegated permissions and no more: read and write the sender's own mail, send as the sender, read the sender's name, and keep the connection alive. The read-and-write permission is what allows the submission to be composed as a draft inside the sender's own mailbox, which is how the sent message lands in their Sent Items and how a carrier's reply threads back to them. It is never granted over anyone else's mailbox, and a test fails the build if a `.Shared` or `.All` scope, or an application permission, is ever added.
