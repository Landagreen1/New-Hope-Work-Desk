...
function generateTiaQuickQuote(input: PdfGenerationInput): PdfGenerationResult {
  ...
  doc.moveDown(0.5);
  doc.fontSize(7).text(`Generated by New Hope Work Desk — ${new Date().toLocaleDateString()}`, { align: 'center' });
...
function generateJsaTruckApplication(input: PdfGenerationInput): PdfGenerationResult {
  ...
  doc.moveDown(0.5);
  doc.text(`Applicant's Signature: ___________________________________     Date: ${new Date().toLocaleDateString()}`);
  doc.moveDown(0.5);
...
function generateGenericApplication(input: PdfGenerationInput): PdfGenerationResult {
  ...
  doc.moveDown(0.5);
  doc.fontSize(9).font('Helvetica').text(`Generated: ${new Date().toLocaleDateString()} | Source: New Hope Work Desk`, { align: 'center' });
  doc.moveDown(1);
