...
export default function CsIntakeLanding({
  ...
  useEffect(() => {
    ...
    getIntake(editId)
      .then((data) => {
        if (data) setSelected({ submission: data.submission, drivers: data.drivers, vehicles: data.vehicles, owners: data.owners, commodities: data.commodities, trailers: data.trailers });
  ...
  if (!['customer_service', 'manager', 'agent', 'commercial', 'commercial_supervisor', 'super_admin', 'sales_supervisor', 'customer_service_supervisor'].includes(profile.role)) {
    return (
