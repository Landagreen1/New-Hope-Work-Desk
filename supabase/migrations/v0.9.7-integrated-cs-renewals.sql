-- New Hope Work Desk v0.9.7
-- Integrated Customer Service Quote Intake + Renewals Management
-- Run this file once in Supabase SQL Editor after the previously verified v0.9.5-r3 migrations.
...
begin
  ...
  loop
    ...
    v_key := null;
    v_title := null;
    if v_days = 30 then v_key := '30'; v_title := 'Renewal enters 30-day window'; end if;
    if v_days = 15 then v_key := '15'; v_title := '15-day renewal warning'; end if;
    if v_days = 7 then v_key := '7'; v_title := '7-day renewal warning'; end if;
    if v_days < 0 then v_key := 'overdue-' || current_date::text; v_title := 'Overdue renewal follow-up'; end if;
...
grant execute on function public.renewal_update_workflow(uuid, text, timestamptz, text) to authenticated;
grant execute on function public.renewal_assign(uuid, uuid) to authenticated;
grant execute on function public.renewal_manager_update(uuid, jsonb) to authenticated;
grant execute on function public.renewal_send_to_requote(uuid) to authenticated;
grant execute on function public.renewal_import_batch(text, jsonb, jsonb) to authenticated;
grant execute on function public.renewal_generate_due_notifications() to authenticated;

grant select on public.renewal_import_runs to authenticated;
grant select on public.renewal_warning_deliveries to authenticated;

commit;

select 'New Hope Work Desk v0.9.7 database update installed' as status;
