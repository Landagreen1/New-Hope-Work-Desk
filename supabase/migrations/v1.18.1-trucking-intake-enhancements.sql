-- ═══════════════════════════════════════════════════════════════════════════════
-- v1.18.1 — Trucking Intake Enhancements
--
-- Adds:
--   1. Physical damage fields on vehicles (value + deductible per truck)
--   2. Truck type field on vehicles
--   3. CDL yes/no + CDL date on drivers
--   4. cargo_coverage_desired on submissions
--   5. Update cs_intake_save_draft to include the new columns
-- ═══════════════════════════════════════════════════════════════════════════════
...
alter table public.cs_intake_vehicles
  add column if not exists physical_damage_value numeric(12,2);

alter table public.cs_intake_vehicles
  add column if not exists physical_damage_deductible numeric(12,2);

-- Truck type (box truck, truck tractor, sprinter van, flatbed, reefer, etc.)
alter table public.cs_intake_vehicles
  add column if not exists truck_type varchar(50);

-- ═══════════════════════════════════════════════════════════════════════════════
-- 2. DRIVER ENHANCEMENTS — CDL
-- ═══════════════════════════════════════════════════════════════════════════════

alter table public.cs_intake_drivers
  add column if not exists cdl boolean default false;

alter table public.cs_intake_drivers
  add column if not exists cdl_date date;

-- ═══════════════════════════════════════════════════════════════════════════════
-- 3. SUBMISSION ENHANCEMENT — Cargo coverage desired
-- ═══════════════════════════════════════════════════════════════════════════════

alter table public.cs_intake_submissions
  add column if not exists cargo_coverage_desired boolean;

-- ═══════════════════════════════════════════════════════════════════════════════
-- 4. UPDATE cs_intake_save_draft to include new columns
--    Signature stays the same (7 params from v1.18.0), only the column lists
--    inside the vehicle and driver inserts change.
-- ═══════════════════════════════════════════════════════════════════════════════
