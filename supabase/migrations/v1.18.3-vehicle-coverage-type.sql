-- ═══════════════════════════════════════════════════════════════════════════════
-- v1.18.3 — Per-vehicle coverage type (Full Coverage / Liability Only)
--
-- Personal auto clients may request full coverage for one car but only
-- liability for another. This adds a coverage_type field to each vehicle row
-- so Customer Service can record the preference per vehicle.
-- ═══════════════════════════════════════════════════════════════════════════════

-- 1. ADD COLUMN
alter table public.cs_intake_vehicles
  add column if not exists coverage_type varchar(20);

comment on column public.cs_intake_vehicles.coverage_type is
  'Per-vehicle coverage preference: full_coverage or liability_only';

-- 2. UPDATE cs_intake_save_draft to persist the new column.
--    Signature stays unchanged (7 params from v1.18.0). Only the vehicle INSERT
--    column list is extended.
