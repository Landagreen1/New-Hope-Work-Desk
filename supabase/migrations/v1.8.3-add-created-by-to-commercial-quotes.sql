-- New Hope Work Desk v1.8.3
-- Add created_by column to commercial_quotes so we can track who originally created the card.
-- cs_intake_submissions already has created_by; no changes needed there.

-- 1. Add the column (nullable so existing rows don't break)
alter table public.commercial_quotes
  add column if not exists created_by uuid references public.profiles(id);

-- 2. Backfill: set created_by = assigned_to for all existing rows where it's null
update public.commercial_quotes
set created_by = assigned_to
where created_by is null;

-- 3. Index for lookups
create index if not exists idx_commercial_quotes_created_by
  on public.commercial_quotes(created_by);

-- 4. Update cs_intake_submit_commercial to also set created_by on the card
...
BEGIN
  ...
  -- Create the commercial card (with created_by tracking who submitted the intake)
  INSERT INTO public.commercial_quotes (
    business_name, description, board_column, column_position,
    risk_level, card_status, coverage_type, assigned_to, is_mirrored, created_by
  ) VALUES (
    v_business_name, v_description, 'quote_intake', v_next_position,
    'medium', 'in_progress', v_coverage_type, v_assigned_to, true, auth.uid()
...
select 'v1.8.3 created_by added to commercial_quotes + RPC updated' as status;
